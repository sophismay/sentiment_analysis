...
class ConnextProcessor(DataProcessor):
  def get_labels(self):
    return ["neg", "pos", "neu"]

  def get_train_examples(self, data_dir):
    print("Getting train examples from {}".format(data_dir))
    return self._create_examples(data_dir, "train.txt")

  def get_dev_examples(self, data_dir):
    print("Getting evaluation examples from {}".format(data_dir))
    return self._create_examples(data_dir, "test.txt")

  def _create_examples(self, data_dir, filename):
    examples = []
    ...
    with tf.gfile.Open(path) as f:
      ...
      for i, line in enumerate(lines):
        ...
        examples.append(
          InputExample(guid= i, text_a= text, text_b= None, label= label)
        )
    print("Returning create examples of length: {}".format(len(examples)))
